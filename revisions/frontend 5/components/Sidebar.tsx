...
const AVAILABLE_ICONS = [
  'Hotel', 'ConciergeBell', 'Bed', 'Utensils', 'Coffee', 'Waves', 'Key',
  'Target', 'Layers', 'PieChart', 'Users', 'Cpu', 'Shield',
...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <aside className={`
      fixed lg:relative inset-y-0 left-0 w-72 bg-white/80 backdrop-blur-xl border-r border-slate-200 flex flex-col h-full shrink-0 z-40 transition-transform duration-300 ease-in-out
    ...
    `}>
      ...
      <nav className="flex-1 overflow-y-auto px-4 space-y-1 custom-scrollbar pb-6">
        <div className="px-3 py-2">
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Operations</span>
        </div>
        ...
        >
          ...
          <span className="text-[13px] tracking-tight">Main Ledger</span>
        </button>
        ...
        {user.role === 'admin' && (
          ...
          >
            ...
            <span className="text-[13px] tracking-tight">Staff Management</span>
          </button>
        )}
        ...
        >
          ...
          <span className="text-[13px] tracking-tight">System Profile</span>
        </button>

        <div className="px-3 pt-6 pb-2">
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Properties & Depts</span>
        </div>
        
        {projects.length === 0 ? (
          <div className="px-3 py-4 text-center">
            <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest leading-relaxed">No active depts</p>
          </div>
        ) : (
          projects.map(project => (
            ...
            >
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 shadow-sm transition-transform group-hover:scale-105 text-white`} style={{ backgroundColor: project.color }}>
                <DynamicIcon name={project.icon || 'Hotel'} size={20} />
              </div>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] p-8 md:p-10 w-full max-w-md shadow-2xl animate-in fade-in zoom-in border border-slate-100">
            <div className="flex items-center justify-between mb-8">
              <h3 className="text-2xl font-black text-slate-800">{editingProject ? 'Modify Dept' : 'Initialize Dept'}</h3>
              <button onClick={() => setShowForm(false)} className="p-2 hover:bg-slate-100 rounded-xl text-slate-400"><X size={24} /></button>
            </div>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Hospitality Icon</label>
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Department Name</label>
                <input autoFocus type="text" required value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="e.g., Front Desk, Room Service" className="w-full px-5 py-4 rounded-2xl bg-slate-50 border-2 border-transparent focus:border-indigo-500 focus:bg-white outline-none transition-all font-bold text-slate-700" />
              </div>
              <button type="submit" className="w-full py-5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-black text-lg transition-all shadow-xl active:scale-[0.98]">
                {editingProject ? 'Update Department' : 'Activate Department'}
              </button>
            </form>
          </div>
        </div>
