              ...
              {projectStats.map((stat) => (
                ...
                >
                  ...
                  <div className="relative h-64 w-full bg-white/40 rounded-3xl border border-slate-100 p-6 flex flex-col group-hover:bg-white group-hover:shadow-lg group-hover:border-indigo-100 transition-all duration-300 backdrop-blur-sm">
                    ...
                    {/* Top IN/OUT Labels (fixed, separate) */}
                    <div className="flex justify-start gap-4 mb-4 px-2">
                      <span className="text-[9px] font-black text-emerald-600">
                        IN: {stat.income.toLocaleString()}
                      </span>
                      <span className="text-[9px] font-black text-rose-600">
                        OUT: {stat.expense.toLocaleString()}
                      </span>
                    </div>
                    ...
                    <div className="flex-1 flex items-end justify-center gap-4 px-2">
                      ...
                      <div
                        ...
                        style={{ width: `${Math.max(32, String(stat.income).length * 8)}px` }}
                      >
                        {/* Moving number above bar */}
                        {stat.income > 0 && (
                          <span
                            className="absolute left-1/2 -translate-x-1/2 text-[8px] font-black text-emerald-600 transition-all duration-1000 pointer-events-none"
                            style={{
                              bottom: `${Math.max((stat.income / chartMax) * 100, 2)}%`,
                              transform: 'translate(-50%, 100%)' // moves just above bar tip
                            }}
                          >
                      ...
                      <div
                        className="flex items-end h-full relative"
                        style={{ width: `${Math.max(32, String(stat.expense).length * 8)}px` }}
                      >
                        {/* Moving number above bar */}
                        {stat.expense > 0 && (
                          <span
                            className="absolute left-1/2 -translate-x-1/2 text-[8px] font-black text-rose-600 transition-all duration-1000 pointer-events-none"
                            style={{
                              bottom: `${Math.max((stat.expense / chartMax) * 100, 2)}%`,
